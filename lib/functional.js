...
"use strict";
	...
	function composeAsync(funcs /*, ...*/) {
		...
		return function composed(x) {
			var context = this;
			return when.reduce(funcs, function(result, f) {
				return f.call(context, result);
			}, x);
		};
	}

	compose.parse = parseCompose.bind(null, compose);
	compose.parseAsync = parseCompose.bind(null, composeAsync);
	 ...
	 */
	function parseCompose(compose, proxy, composeString, wire) {
