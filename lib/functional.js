...
"use strict";
	...
	function weave (f, weaves) {
		return function () {
			return f.apply(this, injectArgs(weaves, [].slice.apply(arguments)));
		}
	}
	 ...
	 */
	function injectArgs (weaves, args) {
		var woven = new Array(Math.max(weaves.length, args.length));
		args = slice(args);
		woven.forEach(function (wat, i) {
			if (i in weaves) {
				args.push(weaves[i], args.shift());
			}
			else {
	...
	function compose(funcs /*, ...*/) {
		...
		if(arguments.length > 1) funcs = slice(arguments);
