...
define(function(require) {
	...
	function instanceFactory(resolver, componentDef, wire) {
		...
		// See https://github.com/cujojs/wire/issues/140
		// You might think that this could simply be:
		// module = wire.loadModule(create);
		// For reasons currently unknown, the above causes IE8 to fail in
		// lib/instantiate when calling ctor.apply() with the error
		// "Object expected".  IE 8's awful debug tools reveal no differences
		// between the constructors or result of Object.create().
		if (typeof create == 'string') {
			create = { module: create };
		}

		if(wire.resolver.isRef(create)) {
			module = wire(create);
		...
		instance = when.join(module, args, isConstructor).spread(createInstance);

		resolver.resolve(instance);

		// Load the module, and use it to create the object
		function createInstance(module, args, isConstructor) {
