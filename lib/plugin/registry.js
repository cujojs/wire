...
define(function(require) {
	...
	var when, array, object, namespaceSeparator;
		...
		scanModule: function(module, spec) {
			...
			if (allowPlugin(module, this.plugins)) {
				...
				return when(module.wire$plugin(this.scopeReady, this.scopeDestroyed, spec),
					function(plugin) {
						plugin && self.registerPlugin(plugin, spec.namespace);
					}
				).yield(module);
			}
	...
	function makeNamespace(pluginName, namespace) {
		return namespace ? (namespace + namespaceSeparator + pluginName) : pluginName;
	}
});
