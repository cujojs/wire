...
define(function(require) {
	...
	function trackInflightRef(refPromise, refGraph, refName, onBehalfOf) {
		...
		refGraph.addEdge(refName, onBehalfOf||'?');

		return timeout(refPromise, refCycleCheckTimeout).then(
			function(resolved) {
				refGraph.removeEdge(refName, onBehalfOf||'?');
				return resolved;
			},
			function() {
	...
	function formatCycles(cycles) {
		return cycles.map(function (sc) {
			// Artificially duplicate the last vertex in the cycle solely for
			// formatting purposes.  This produces output like a -> b -> a
			// rather than just b -> a
			sc.unshift(sc[sc.length - 1]);
