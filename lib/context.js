...
function(require, when, array, object, createModuleLoader, Lifecycle, Resolver, basePlugin) {
	...
	function createScope(scopeDef, parent, options) {
		var scope, scopeParent, config, initializers, finalizers, destroyers,
		...
		function executeInitializers() {
			return sequence(initializers, components, scopeDef);
		}
		function executeFinalizers() {
			return sequence(finalizers, components, scopeDef);
		}
		function executeDestroyers() {
			return sequence(destroyers, components, scopeDef);
		}
		...
		function configureContext(options) {
			...
			initializers = array.delegate(options.initializers);
			finalizers = array.delegate(options.finalizers);
			destroyers = array.delegate(options.destroyers);
		}
		...
		function parseSpec(scopeDef, scopeReady) {
			...
			chain(whenAll(promises, executeFinalizers), scopeReady, components);
		}
		...
		function wireFactory(resolver, spec, wire) {
			...
			function createChild(/** {Object|String}? */ mixin) {
				...
				return wireChild(spec, { initializers: [init] });
			}
	...
	function sequence(tasks) {
		...
		return when.reduce(tasks, function(context, initializer) {
			return when(initializer.apply(context, args), function() {
				return context;
			});
		}, undef);
	}
