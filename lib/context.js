...
(function(define){
define(['require', 'when', 'when/timeout', './array', './object', './async', './loader', './lifecycle', './resolver', './proxy', '../base'],
function(require, when, timeout, array, object, async, loader, Lifecycle, Resolver, proxy, basePlugin) {
	...
	function createContext(specs, parent, options) {
		...
		return moduleLoader.merge(specs).then(
			function(spec) {
				return createScope(spec, parent, options)
					.then(function(scope) { return scope.components; });
			}
		);
	}
	...
	function createScope(scopeDef, parent, options) {
		...
		function inheritFromParent(parent, options) {
			...
			descriptors = object.inherit(parent.descriptors);
			...
			scopeParent.descriptors = descriptors;
		...
		function scanPlugin(module, spec) {
			if (module && typeof module.wire$plugin == 'function' && plugins.indexOf(module.wire$plugin) === -1) {
		...
		function realizeComponent(spec, name) {
			...
			return when(findFactory(spec),
				...
				function () {
					...
					return createScope(spec, scope, { name: name }).then(function(childScope) {
		...
		function createProxy(component, spec) {
			return when(modulesReady, function() {
				...
				componentProxy = proxiers.reduce(function(proxy, proxyHandler) {
