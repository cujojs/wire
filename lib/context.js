...
(function(define){
define(['require', 'when', './array', './object', './moduleLoader', './lifecycle', '../base'],
function(require, when, array, object, createModuleLoader, Lifecycle, basePlugin) {
	...
	function createScope(scopeDef, parent, options) {
		var scope, scopeParent, proxiedComponents, objects, lifecycle,
		...
		function doResolveRef(refName, refObj, scope) {
			...
			return refName in scope
				...
				: when(modulesReady, function () {
					return resolveWithPlugin(refName, refObj);
				});
		}

		function resolveWithPlugin(refName, refObj) {
