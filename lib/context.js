...
(function(define){
define(['require', 'when', './array', './object', './moduleLoader', '../base'],
function(require, when, array, object, createModuleLoader, basePlugin) {
	...
	function wireContext(specs, parent, options) {
		...
		return when(ensureAllSpecsLoaded(specs, moduleLoader), doWireContexts);
	}
	...
	function createScope(scopeDef, parent, options) {
		var scope, scopeParent, local, proxiedComponents, objects,
		...
		function initFromParent(parent, options) {
			...
			objects = initWireApi(object.inherit(parent.objects));
			...
			return when(findFactory(spec),
				function (factory) {
					...
					return when(factoryPromise, function(component) {
						return processObject(createProxy(component, spec));
					});
				},
				function () {
