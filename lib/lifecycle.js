...
define(function(require) {
	...
	function createLifecyclePhase(steps) {
		...
		return function(proxy, metadata) {
			var self = this;
			return when.reduce(steps, function (unused, step) {
				return processFacets(step, proxy, metadata, self._config);
			}, proxy);
		};
	}

	function processFacets(step, proxy, metadata, config) {
		var promises, options, name, spec, facets, safeNames, unprocessed;
