...
define(function (require) {
	...
	function createReactPoints (nodes, options) {
		...
		stringify = options.transform;

		if (!stringify) throw new Error('options.transform not specified.');

		return nodes.reduce(function (list, node) {
			var reactAttr, pointDefs;

			reactAttr = node.getAttribute('data-wire-reactpoint');
			if (!options.preserveAttrs) node.removeAttribute('data-wire-reactpoint');
