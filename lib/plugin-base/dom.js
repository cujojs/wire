...
define(function(require) {
	...
	var domReady, when, base, on, object, WireProxy, NodeProxy, priority;

	when = require('when');

	domReady = require('wire/domReady');
	base = require('../dom/base');
	on = require('../dom/on');
	object = require('../object');
	WireProxy = require('../WireProxy');
	NodeProxy = require('../dom/NodeProxy');
	priority = require('../plugin/priority');

	function getElementFactory (resolver, componentDef, wire) {
		var promise = wire(componentDef.options).then(function (element) {
	...
	return function createDomPlugin(options) {
		...
		var getById, query, first, addClass, removeClass, placeAt, doOn,
			doById, doPlaceAt, resolveQuery, nodeProxy;
		...
		nodeProxy = Object.create(NodeProxy.prototype);
		nodeProxy.on = function(eventString, handler) {
			return doOn(eventString, this.target, handler);
		};

		proxyNode.priority = priority.basePriority;
		function proxyNode (proxy) {

			if (!base.isNode(proxy.target)) {
				return proxy;
			}

			return WireProxy.extend(proxy, nodeProxy);
		}
