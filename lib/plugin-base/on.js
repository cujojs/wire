...
"use strict";
	...
	return function createOnPlugin (options) {
				...
				function parseIncomingOn(source, targetProxy, connections, wire) {
					...
					function addConnection(pairs, source, proxy, func) {
						removers = removers.concat(
							registerHandlers(pairs, source, function() {
								return proxy.invoke(func, arguments);
							}, prevent, stop)
						);
					}
					...
					return when(wire.resolveRef(refName),
						...
						function () {
							...
							function createConnection(source, eventsString, handler) {
								...
								removers = removers.concat(
									registerHandlers(pairs, source, handler, prevent, stop)
								);
							}
						}
					);
		...
		function registerHandlers (pairs, node, handler, prevent, stop) {
			var removers, handler;
			removers = [];
			for (var i = 0, len = pairs.length; i < len; i++) {
				handler = makeEventHandler(handler, prevent, stop);
