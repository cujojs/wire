...
"use strict";
	...
	return function createOnPlugin (options) {
		 ...
		 * are pre-configured with one or more selectors and optionally one
		 ...
		 *     function (rootNode, eventName, handler) {}
		 */
		function createOnResolver (eventSelector) {
			var fallbackEvents;
			// split event/selector string
			fallbackEvents = splitEventSelectorString(eventSelector, '');
			return function () {
				var args, node, events, handler, unwatches;
				// resolve arguments
				args = Array.prototype.slice.call(arguments, 0, 3);
				handler = args.pop();
				node = args.shift() || document;
				events = args[0] ? [args[0]] : fallbackEvents;
				if (!events) throw new Error('on! resolver: event is undefined');
				unwatches = [];
				// for each event
				events.forEach(function (event) {
					// create a handler
					unwatches.push(on(node, event, handler, fallbackEvents.selector));
				});
				// return unwatcher
				return function () {
	...
	function splitEventSelectorString (string, defaultSelector) {
		...
		if (selectors) events.selector = selectors;
