...
define(function(require) {
	...
	var when, sequence, array, object, async, loader, Lifecycle, Resolver,
		WireProxy, createPluginRegistry, defaultPlugins,
	...
	scope = {
		...
		_makeResolvable: function(metadata, component) {
			var id = metadata.id;
			if(id != null) {
				this.components[id] = WireProxy.getTarget(async.getValue(component));
			}
		},
		...
		_createComponent: function(spec, metadata) {
			...
			return when(this._findFactory(spec),
				function (found) {
					...
					return component.promise.then(function(created) {
						return self._proxyComponent(created, metadata);
					})
					.then(function(proxy) {
		...
		_proxyComponent: function(component, metadata) {
			var proxy, proxiers, self;

			if (WireProxy.isProxy(component)) {
				proxy = component;
				component = WireProxy.getTarget(proxy);
			} else {
				proxy = WireProxy.create(component);
			}

			proxy = WireProxy.init(proxy, this.lifecycle, metadata);
			...
			return this.modulesReady.promise.then(function() {
				...
				self._registerProxy(proxy, metadata);

				return proxy;
			});
		},

		_registerProxy: function(proxy, metadata) {
			if(metadata) {
				proxy.path = metadata.path = this._createPath(metadata.id);
