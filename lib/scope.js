...
(function(define){
define(['require', 'when', 'when/timeout', './array', './object', './async', './loader', './lifecycle', './resolver', './proxy', './plugin/registry', './plugin/defaultPlugins'],
function(require, when, timeout, array, object, async, loader, Lifecycle, Resolver, proxy, createPluginRegistry, defaultPlugins) {
	...
	function createScope(scopeDef, parent, options) {
		var scope, scopeParent, context, config, contextHandlers, createContext,
			...
			plugins, pluginApi, moduleLoader, modulesToLoad,
		...
		defaultPlugins.forEach(scanPlugin);
		...
		function inheritFromParent(parent, options) {
			...
			plugins = createPluginRegistry(parent.pluginRegistry||{}, scopeReady.promise, scopeDestroyed.promise);
		...
		function createPluginApi() {
			...
			pluginApi.createChild = wireChild;
		...
		function makeResolvable(name, component) {
			components[name] = async.getValue(component);
		}
		...
		function scanPlugin(module, spec) {
			return plugins.scanModule(module, spec);
		}
		...
		function createProxy(component, spec) {
			return when(modulesReady, function() {
				...
				componentProxy = plugins.proxiers.reduce(function(proxy, proxyHandler) {
