	...
	scope = {
		...
		getProxy: function(nameOrComponent, onBehalfOf) {
			...
			return typeof nameOrComponent == 'string'
				? when(this._resolveRefName(nameOrComponent, {}, onBehalfOf), function (component) {
					return self._proxyInstance(component);
				})
				: self._proxyInstance(nameOrComponent);
		},
		...
		_initPluginApi: function() {
			...
			pluginApi.contextualize = function(name) {
				function contextualApi(spec, id, path) {
					var componentDef = self._createComponentDef(id);
					return self._resolveItem(spec, componentDef);
				}

				contextualApi.addInstance = function(instance, id) {
					var componentDef = self._createComponentDef(id);
					return self._processInstance(instance, componentDef);
				};
		...
		_createArray: function(arrayDef, component) {
			var self, id, i;
			...
			return when.map(arrayDef, function(item) {
				var componentDef = self._createComponentDef(id + '[' + (i++) + ']');
				return self._resolveItem(item, componentDef);
			});
		},
			...
			return when(this._findFactory(spec),
				function (found) {
					...
					return instance.promise.then(function(instance) {
						return self._processInstance(instance, component);
					});
				},
				function () {
		...
		_processInstance: function(instance, component) {
			var self = this;

			return this._proxyInstance(instance, component)
				.then(function(proxy) {
		...
		_proxyInstance: function(instance, component) {
