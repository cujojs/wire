	...
	WireScope.prototype = {
		...
		init: function(spec, parent) {
			...
			function prepareScope() {
				var parsed = self._parseSpec(spec).then(contextEvent('ready'));
				ready.resolve(parsed.yield(self));
				self._createComponents();
			}

			function finalizeScope() {
				return self._ensureAllModulesLoaded()
					.then(contextEvent('initialize'))
					.then(self.modulesReady.resolve);
			}
		...
		_configure: function() {
			...
			this.componentFactory = this._createComponentFactory(
				this.lifecycle, this.modulesReady.promise, config);
		},
		...
		_createComponentFactory: function(lifecycle, modulesReady, config) {
			var componentFactory, initProxy, getFactory;

			componentFactory = new ComponentFactory(lifecycle, config);

			initProxy = componentFactory.initProxy;
			componentFactory.initProxy = function() {
				var args = array.fromArguments(arguments);
				return modulesReady.then(function() {
					return initProxy.apply(componentFactory, args);
				})
			};

			getFactory = componentFactory.getFactory;
			componentFactory.getFactory = function() {
				var self, found, args;

				self = this;
				args = array.fromArguments(arguments);
				found = getFactory.apply(componentFactory, args);

				if(!found) {
					found = modulesReady.then(function() {
						return getFactory.apply(componentFactory, args)
							|| when.reject('No factory: ' + args[0]);
					});
				}
		...
		_ensureAllModulesLoaded: function() {
			var self = this;
			return async.until(waitForModules, 0, allModulesLoaded);
