...
define(function(require) {
	...
	WireScope.prototype = {
		...
		init: function(spec, parent) {
			var self, destroyed, contextEventApi, startupSequence, parsed;
			...
			startupSequence = [
				function() {
					return self._executeInitializers();
				},
				function(parsed) {
					return self._installPlugins(parsed.plugins)
				},
				contextEvent('initialize'),
				function(parsed) {
			...
			parsed = self._parseSpec(spec);

			return sequence(startupSequence, parsed).yield(self);
		...
		_installPlugins: function(plugins) {
			...
			function installPlugin(pluginSpec, namespace) {
				...
				if(typeof pluginSpec == 'string') {
					...
					module = pluginSpec;
				} else if(typeof pluginSpec == 'function') {
					module = { wire$plugin: pluginSpec };
				}

//				return self._createComponent(self._createComponentDef('', plugin))
		...
		_createScopeItem: function(component) {
			...
			return when(item, function (resolved) {
