...
define(function(require) {
	...
	scope = {
		...
		_makeResolvable: function(metadata, component) {
			this.components[metadata.id] = async.getValue(component);
		},
		...
		_resolveItem: function(spec, itemMetadata) {
			...
			if (this.resolver.isRef(spec)) {
				// Reference
				item = this._resolveRef(spec, itemMetadata.name);
			} else {
				// Component
			...
			return when(this._findFactory(spec),
				function (factory) {
					...
					return when(component.promise, function(createdComponent) {
						return self._processComponent(createdComponent, spec, componentMetadata)
